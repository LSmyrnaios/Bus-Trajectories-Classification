...
def read_dataset(shouldReadTrain, shouldReadTestA1, shouldReadTestA2, dynamic_datasets_path=''):
    ...
    if shouldReadTestA1:
        ...
        print("Going to load training-set: \'" + locationTestA1 + "\'")
        testSetA1 = pd.read_csv(locationTestA1,
                                converters={"Trajectory": literal_eval},
                                sep="\t"
                                )
        print("Finished loading training-set. Its size is: " + testSetA1.__len__().__str__() + " records.")
        returnValues.append(testSetA1)

    if shouldReadTestA2:
        locationTestA2 = os.path.join(dynamic_datasets_path, 'Resources', 'DataSets', 'test_set_a2.csv')
        print("Going to load training-set: \'" + locationTestA2 + "\'")
        testSetA2 = pd.read_csv(locationTestA2,
                                converters={"Trajectory": literal_eval},
                                sep="\t"
                                )
        print("Finished loading training-set. Its size is: " + testSetA2.__len__().__str__() + " records.")
