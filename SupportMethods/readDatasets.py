...
def read_dataset(shouldReadTrain, shouldReadTestA1, shouldReadTestA2, dynamic_path=''):
    returnValues = []

    if shouldReadTrain:
        locationTrain = dynamic_path + 'Resources/DataSets/train_set.csv'
        trainSet = pd.read_csv(locationTrain,
                               converters={"Trajectory": literal_eval},
                               index_col='tripId'
                               )
        print("Finished loading train: \'" + "train_set.csv" + "\'")
        print('TrainSet size: ' + trainSet.__len__().__str__())
        returnValues.append(trainSet)

    if shouldReadTestA1:
        locationTestA1 = dynamic_path + 'Resources/DataSets/test_set_a1.csv'
    ...
    if shouldReadTestA2:
        locationTestA2 = dynamic_path + 'Resources/DataSets/test_set_a2.csv'
